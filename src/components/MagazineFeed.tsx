...
export default function MagazineFeed({ posts, categories, activeCategory, onCategoryChange, loading }: MagazineFeedProps) {
  ...
  return (
    <section className="container-wide py-16">
      ...
      {/* Category Filter Bar */}
      <div className="sticky top-20 z-20 -mx-6 px-6 py-4 bg-porcelain/90 backdrop-blur-sm mb-10 border-b border-taupe/30">
      ...
      {loading ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-12">
          {posts.map((post, i) => (
            ...
            >
              <div className="img-zoom rounded-sm overflow-hidden luxury-shadow aspect-[4/5] relative">
                {post.cover_image && (
                ...
                <div className="absolute top-4 right-4 w-8 h-8 rounded-full bg-white/80 backdrop-blur-md flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-400">
                  <Bookmark size={14} strokeWidth={1.5} className="text-charcoal" />
                </div>
              </div>
              <div className="mt-5">
                {post.author && (
