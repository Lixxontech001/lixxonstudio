...
const TOPICS = ['Editorial Pitch', 'Brand Partnership', 'Product Review Request', 'General Inquiry'];
...
export default function ContactForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-5 mt-10">
      ...
      <div>
        <label className="block text-[10px] tracking-editorial uppercase text-charcoal-muted mb-2">Topic</label>
        <div className="flex flex-wrap gap-2">
          {TOPICS.map(topic => (
            <button
              ...
              className={`px-4 py-2.5 text-xs tracking-editorial uppercase font-medium rounded-sm border transition-all duration-300 ${
                form.topic === topic
                  ? 'bg-charcoal text-white border-charcoal'
                  : 'bg-transparent text-charcoal-muted border-taupe/50 hover:border-bronze hover:text-charcoal'
              }`}
            >
              {topic}
            </button>
